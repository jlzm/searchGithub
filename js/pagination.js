let pagination_wrap, config, page_amount, page_range, pagination_list;
...
function calc_page_range() {
    let start_page, middle = Math.ceil(config.page_range / 2),
        end_page;
...
function calc_page_amount() {
    if (Math.ceil(config.amount / config.limit) > 200)
        return page_amount = 200;
